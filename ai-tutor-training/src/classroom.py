        ...
        if self.state == ConversationState.TEACHER_TURN:
            ...
            if self.type == ConversationType.ATTEMPTED and len(self.conversation) == 0:
                return [
                    ...
                    {"role": "user", "content": "Please provide your solution to this problem."}
                ]
            conversation = [
...
class Classroom:
    def __init__(
        ...
        if self.teacher_model_cfg.use_openrouter:
        ...
        elif self.teacher_model_cfg.use_customed_api:
            self.teacher_model = CustomedAPIInference(
                self.teacher_model_cfg.model_name_or_path
            )
        else:
            self.teacher_model = ParallelvLLMInference(
                ...
                enable_thinking=generation_cfg.use_thinking
            )
        ...
        elif self.student_model_cfg.use_customed_api:
            self.student_model = CustomedAPIInference(
                self.student_model_cfg.model_name_or_path
            )
        else:
        ...
        elif self.judge_model_cfg.use_customed_api:
            self.judge_model = CustomedAPIInference(self.judge_model_cfg.model_name_or_path)
        else:
