...
def _load_weights(worker, weights):
...
class ParallelvLLMInference:
    ...
    def run_batch(
        ...
        if current is not None and current != self._last_reload_ckpt:
            logger.info(
                f"New lora checkpoint {current} detected (was {self._last_reload_ckpt})."
            )
            # self._reload_workers()
            new_path = os.path.join(self.model_save_path, f"checkpoint-{current}")
            logger.info(f"Reload: switching lora_path to {new_path}")
            self.lora_path = new_path
            self._last_reload_ckpt = current
    ...
    def _handle_causallm_task(
        ...
        if meta is not None and meta.get("use_shared_memory", False):
            # from ..utils.shared_memory import load_shared_state_dict

            # state = load_shared_state_dict(meta)
            # llm.collective_rpc(_load_weights, args=(state,))
            # return llm.chat(prompts, sampling_params=sampling_params, 
            #                 chat_template_kwargs={"enable_thinking": self.enable_thinking})

            lora_request = LoRARequest(
                lora_name=f"dynamic_lora", 
                lora_int_id=1, 
                lora_path="/dev/shm/lora_ckpt",
            )
