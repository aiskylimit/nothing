...
from src.utils.tele_bot import TrainingWatchdog
...
class ClassroomGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        model = get_peft_model(base_model, peft_config) if peft_config is not None else base_model
        ...
        if self.beta == 0.0:
            ...
            # self.ref_model = AutoModelForImageTextToText.from_pretrained(
            #     self.model_name_or_path, **model_init_kwargs
            # )
    ...
    def _get_last_hidden_state(
        ...
        last_hidden_state = unwrapped_model.model.model(
    ...
    def _save_only_model(self, output_dir: str, max_keep: int = 10):
        ...
        if self.accelerator.is_main_process:
            ...
            policy_root = os.path.dirname(output_dir)
            
            valid_checkpoints = []
            paths_to_delete = []
            
            for d in os.listdir(policy_root):
                match = CHECKPOINT_RE.match(d)
                if match:
                    step = int(match.group(1))
                    full_path = os.path.join(policy_root, d)
            ...
            if len(valid_checkpoints) > max_keep:
                num_to_delete = len(valid_checkpoints) - max_keep
                for step, path in valid_checkpoints[:num_to_delete]:
    ...
    def train(self, *args, **kwargs):
        try:
            ...
            if self.is_world_process_zero():
                ...
                msg = (f"🚨 <b>TRAINING BỊ CRASH!</b>\n"
                       f"<b>Lỗi:</b> {str(e)}\n\n"
                       f"<b>Chi tiết:</b>\n<code>{error_trace[-10:]}</code>")
                self.watchdog.send_mess(msg)
