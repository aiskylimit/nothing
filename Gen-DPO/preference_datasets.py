...
def _make_chat_messages(prompt: str, response: Optional[str] = None) -> List[Dict[str, str]]:
...
def _make_sequence_weight(
    prompt_len: int,
    sequence_input_ids: List[int],
    response_input_ids: List[int],
    response_weight,
    default_weight: float = 1.0,
) -> List[float]:
    weights = [0] * len(sequence_input_ids)
    response_start = _find_subsequence(sequence_input_ids[prompt_len:], response_input_ids)
    if response_start is None:
        response_start = max(0, len(sequence_input_ids) - prompt_len - len(response_input_ids))

    response_start += prompt_len
    if response_weight is None:
        response_weight = [default_weight] * len(response_input_ids)

    for offset, weight in enumerate(response_weight):
        index = response_start + offset
...
def tokenize_batch_element(prompt: str, chosen: str, rejected: str, truncation_mode: str, tokenizer, max_length: int, max_prompt_length: int, rejected_weight=None, chosen_weight=None) -> Optional[Dict]:
    """Tokenize a single batch element.
    
       At this stage, we don't convert to PyTorch tensors yet. We apply the
         tokenizer chat template to prompt+chosen, prompt+rejected, and prompt
         alone, then discard samples whose templated sequence exceeds max_length.

       We also create labels for the chosen/rejected responses, with -100 for
         the prompt tokens.
    """
    raw_prompt_tokens = tokenizer(prompt, add_special_tokens=False)
    raw_chosen_tokens = tokenizer(chosen, add_special_tokens=False)
    raw_rejected_tokens = tokenizer(rejected, add_special_tokens=False)

    if rejected_weight is not None:
        assert len(rejected_weight) == len(raw_rejected_tokens['input_ids'])
    
    if chosen_weight is not None:
        assert len(chosen_weight) == len(raw_chosen_tokens['input_ids'])
    ...
    prompt_message = _make_chat_messages(prompt)
    chosen_message = _make_chat_messages(prompt, chosen)
    rejected_message = _make_chat_messages(prompt, rejected)
    ...
    prompt_template_message = tokenizer.apply_chat_template(
        prompt_message, add_generation_prompt=False, tokenize=False
    )

    prompt_tokens = tokenizer(prompt_template_message, add_special_tokens=False)
    ...
    batch['rejected_weight'] = _make_sequence_weight(
        len(prompt_tokens['input_ids']),
        rejected_sequence_tokens['input_ids'],
        raw_rejected_tokens['input_ids'],
        rejected_weight,
    )
    batch['chosen_weight'] = _make_sequence_weight(
        len(prompt_tokens['input_ids']),
        chosen_sequence_tokens['input_ids'],
        raw_chosen_tokens['input_ids'],
        chosen_weight,
    )
