...
def gendpo_loss(chosen_values: torch.FloatTensor,
                rejected_values: torch.FloatTensor,
                beta: float = 0.3) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
    # TDPO1-style, không detach bên nào — nhưng scale đều bằng alpha để kiểm soát cường độ
    # alpha=1.0 -> đúng bản "Gen-DPO(new)" hiện tại
    # alpha=0.0 -> đúng bản Gen-DPO cũ (không có KL term)
...
def _get_batch_gendpo_values(logits: torch.FloatTensor,
                             reference_logits: torch.FloatTensor,
                             labels: torch.LongTensor,
                             gamma: float = 0.1,) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
    assert logits.shape[:-1] == labels.shape
    assert reference_logits.shape[:-1] == labels.shape
    ...
    token_logps = torch.gather(vocab_logps, dim=2, index=labels.unsqueeze(2)).squeeze(2)
    reference_token_logps = torch.gather(reference_vocab_logps, dim=2, index=labels.unsqueeze(2)).squeeze(2)

    pi_theta = token_logps.exp()
    log_ratios = token_logps - reference_token_logps
    # giữ nguyên omega gốc như bản cũ
    omega = (gamma / (pi_theta.detach() + gamma))


    values = (omega * log_ratios * loss_mask).sum(-1)
    logps = (token_logps * loss_mask).sum(-1)
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1, transform_config = None):
        ...
        # estimate total training steps for scheduler (required for cosine schedule)
        if config.n_examples is not None:
            num_training_steps = math.ceil(config.n_examples / config.batch_size)
        else:
            total_examples_in_dataset = 0
            for name in config.datasets:
                data = get_dataset(name, 'train', silent=True, transform_config=transform_config, base_data_dir=config.base_data_dir)
                for _, d in data.items():
                    total_examples_in_dataset += len(d.get('pairs', []))
            num_training_steps = max(1, math.ceil(total_examples_in_dataset * config.n_epochs / config.batch_size))
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        if self.config.loss.name in {'dpo', 'ipo', 'tdpo', 'tisdpo', 'gendpo'}:
    ...
    def gendpo_concatenated_forward(self, model: nn.Module, reference_model: nn.Module,
                                    batch: Dict[str, Union[List, torch.LongTensor]],
                                    gamma: float = 0.1,):
        concatenated_batch = concatenated_inputs(batch)
        all_logits = model(concatenated_batch['concatenated_input_ids'],
        ...
        all_style_values, all_logps = _get_batch_gendpo_values(
            all_logits,
        ...
        chosen_values = all_style_values[:batch_size]
        rejected_values = all_style_values[batch_size:]
        chosen_logps = all_logps[:batch_size].detach()
        rejected_logps = all_logps[batch_size:].detach()

        return chosen_values, rejected_values, chosen_logps, rejected_logps
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            policy_rejected_logps = all_gather_if_needed(policy_rejected_logps.detach(), self.rank, self.world_size)
            metrics[f'logps_{train_test}/rejected'] = policy_rejected_logps.cpu().numpy().tolist()
        elif loss_config.name == 'gendpo':
            chosen_values, rejected_values, policy_chosen_logps, policy_rejected_logps = self.gendpo_concatenated_forward(
                self.policy,
                self.reference_model,
                batch,
                gamma=loss_config.get('gamma', 0.1),
            )
            losses, chosen_rewards, rejected_rewards = gendpo_loss(
    ...
    def _logits_sam_step(self, batch, loss_config, rho: float):
        self.optimizer.zero_grad()

        lm_head = self.policy.lm_head
        W = lm_head.weight

        tied = W.data_ptr() == self.policy.model.embed_tokens.weight.data_ptr()
        if tied:
            # Tách tie tạm thời: clone data vào W.data (không tạo Parameter mới)
            W.data = W.data.clone()
        ...
        for microbatch_idx in range(self.config.gradient_accumulation_steps):
            ...
            loss, metrics = self.get_batch_metrics(local_microbatch, loss_config, train=True)
            (loss / self.config.gradient_accumulation_steps).backward()
            for k, v in metrics.items():
                batch_metrics[k].extend(v)

        g = W.grad.detach()
        perturbation = rho * g / (g.norm() + 1e-12)
        W.data.add_(perturbation)

        self.optimizer.zero_grad()
        for microbatch_idx in range(self.config.gradient_accumulation_steps):
            global_microbatch = slice_and_move_batch_for_device(batch, microbatch_idx, self.config.gradient_accumulation_steps, self.rank)
            local_microbatch = slice_and_move_batch_for_device(global_microbatch, self.rank, self.world_size, self.rank)
            loss, _ = self.get_batch_metrics(local_microbatch, loss_config, train=True)
            (loss / self.config.gradient_accumulation_steps).backward()

        W.data.sub_(perturbation)

        # Restore tie: trỏ W.data về embed_tokens (không tạo Parameter mới)
        if tied:
            W.data = self.policy.model.embed_tokens.weight.data
    ...
    def train(self):
        ...
        # Use cosine scheduler with 5% warmup period
        num_warmup_steps = max(1, int(0.05 * self.num_training_steps))
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                # Keep only the final model save at the end of training.
            #### END EVALUATION ####
            ...
            sam_rho = getattr(self.config, 'sam_rho', 0.0)

            if sam_rho > 0:
                batch_metrics = self._logits_sam_step(batch, self.config.loss, sam_rho)
            else:
                batch_metrics = defaultdict(list)
                for microbatch_idx in range(self.config.gradient_accumulation_steps):
                    global_microbatch = slice_and_move_batch_for_device(batch, microbatch_idx, self.config.gradient_accumulation_steps, self.rank)
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1, transform_config = None):
        ...
        if config.loss.name in {'dpo', 'ipo', 'tdpo', 'tisdpo', 'gendpo'}:
    ...
    def _logits_sam_step(self, batch, loss_config, rho: float):
        ...
        # Với FSDP, dùng summon_full_params để access lm_head
        with FSDP.summon_full_params(self.policy, writeback=True, recurse=True):
            lm_head = self.policy.lm_head
            W = lm_head.weight

            g = W.grad
            if g is None:
                raise RuntimeError("lm_head.weight.grad is None sau backward. Kiểm tra FSDP config.")
            g = g.detach().clone()
            perturbation = rho * g / (g.norm() + 1e-12)
            W.data.add_(perturbation)
        ...
        with FSDP.summon_full_params(self.policy, writeback=True, recurse=True):
            self.policy.lm_head.weight.data.sub_(perturbation)
