...
class AVSDTrainer(SFTTrainer):
    _tag_names = ["trl", "avsd"]
    _name = "AVSD"
    # PVSDTrainer sets this to True: its teacher is the steered student, so no
    # teacher sequence over the rollout is built.
    pvsd_enabled = False
    ...
    def training_step(
        ...
        if self.pvsd_enabled:
            # PVSD's teacher is the student steered by a privilege vector, so there
            # is no teacher sequence to build over the rollout.
            pass
