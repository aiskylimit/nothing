...
# Datasets are written outside the code folder (remote-runner rule).
DATA_DIR="${DATA_DIR:-${PVSD_OUT_ROOT}/data}"
mkdir -p "${DATA_DIR}"

cmd=(
    python -m pvsd.code.download_codeforces_cots_py
    --train-output "${TRAIN_OUTPUT:-${DATA_DIR}/codeforces_cots_py_train.jsonl}"
    --eval-output "${EVAL_OUTPUT:-${DATA_DIR}/codeforces_cots_py_eval.jsonl}"
