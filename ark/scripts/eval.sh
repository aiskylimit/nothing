...
# python evaluation.py --model octopus --model_args pretrained=checkpoints/llama-8b-alpaca-cleaned,dtype=float16 \
...
python evaluation.py --model octopus \
  --model_args pretrained=checkpoints/llama-8b-alpaca-cleaned,dtype=float16 \
  --tasks hellaswag,arc_easy,arc_challenge --num_fewshot 0 \
  --device cuda --batch_size 16 \
  --output_path results/llama-8b-alpaca-cleaned-combined.json
