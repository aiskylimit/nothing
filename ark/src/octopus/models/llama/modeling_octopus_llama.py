...
from transformers.models.llama.modeling_llama import apply_rotary_pos_emb, create_causal_mask

from octopus.cache_utils import OctopusDynamicCache
from .configuration_octopus_llama import OctopusLlamaConfig
...
class OctopusLlamaAttention(LlamaAttention):
    """OctopusLlamaAttention is a wrapper around LlamaAttention that adds octopus-specific functionality.
    """
    
    def __init__(self, config: OctopusLlamaConfig, layer_idx: int):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Note: Llama doesn't have q_norm/k_norm unlike Qwen3
        ...
        # if not use_base_attention:
            ...
            # portion_scores = gated_states.float() / (gated_states.float().sum(dim=-1, keepdim=True) 
            ...
            # attention_weights = (gated_attention_weights + redistributed_weights).to(dtype=attention_weights.dtype)
            # attention_weights = gated_attention_weights.to(dtype=attention_weights.dtype)
...
class OctopusLlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: OctopusLlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = OctopusLlamaAttention(config=config, layer_idx=layer_idx)
...
class OctopusLlamaModel(LlamaPreTrainedModel):
    def __init__(self, config: OctopusLlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [OctopusLlamaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class OctopusLlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = {"lm_head.weight": "model.embed_tokens.weight"}
    _tp_plan = {"lm_head": "colwise_rep"}
    _pp_plan = {"lm_head": (["hidden_states"], ["logits"])}
    _no_split_modules = ["OctopusLlamaDecoderLayer"]
    
    def __init__(self, config):
        super().__init__(config)
        self.model = OctopusLlamaModel(config)
        self.vocab_size = config.vocab_size
