...
from exec_eval import TIMEOUT as DEFAULT_EXEC_TIMEOUT
...
def evaluate(
    ...
    pbar = tqdm(total=sum(len(session) for session in glist), desc="Evaluating", unit="query") \
        if progress_bar_for_each_datapoint else None
    log = tqdm.write if pbar is not None else print
    for i, (p, g) in enumerate(zip(plist, glist)):
        if not progress_bar_for_each_datapoint and (i + 1) % 10 == 0:
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "match"]:
                ...
                if exact_score == 0:
                    ...
                    log("{} pred: {}".format(hardness, p_str))
                    log("{} gold: {}".format(hardness, g_str))
                    log("")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exec_timeout', type=int, default=DEFAULT_EXEC_TIMEOUT,
    ...
    evaluate(args.gold, args.pred, args.db, args.etype, kmaps, args.plug_value, args.keep_distinct,
             args.progress_bar_for_each_datapoint, args.exec_timeout)
