...
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/qwen_ablation_2_1}"
...
script_for_variant() {
  case "$1" in
    prompt_vs_student_response)
      echo "scripts/qwen_ablation_2_1/train_prompt_vs_student_response.sh"
      ;;
    *)
      echo "Unknown ablation variant: $1" >&2
      echo "Supported variants: prompt_vs_student_response" >&2
      exit 2
      ;;
  esac
}
...
for variant in "${VARIANTS[@]}"; do
  ...
  bash running.sh \
    ...
    --skip-finalize \
    --filter "${train_script}" \
    --log-dir "run_logs/qwen_ablation_2_1/${variant}" \
    --infer-after-train \
    ...
    --infer-output-root "${INFER_OUTPUT_ROOT}/${variant}" \
