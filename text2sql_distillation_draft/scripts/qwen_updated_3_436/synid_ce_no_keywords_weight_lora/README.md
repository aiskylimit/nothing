...
SynID-CE CSD alpha/beta sweep on Spider privileged LoRA-436 augmented data without SQL keyword/schema token up-weighting during semantic-anchor pooling. Alpha and beta use the same value in each grid item.
...
```bash
bash scripts/qwen_updated_3_436/synid_ce_no_keywords_weight_lora/run_full_pipeline.sh
```
