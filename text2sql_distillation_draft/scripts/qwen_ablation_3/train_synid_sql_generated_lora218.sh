...
SYNID_USE_PRIVILEGED_TEACHER_INPUT="${SYNID_USE_PRIVILEGED_TEACHER_INPUT:-false}"
...
check_train_only_indexed_dataset() {
  local data_dir="${1%/}"
  local missing=0
  ...
  if [[ "${SYNID_USE_PRIVILEGED_TEACHER_INPUT}" =~ ^(0|false|no|n)$ ]]; then
    if [[ -e "${data_dir}/teacher_train_0.bin" || -e "${data_dir}/teacher_train_0.idx" ]]; then
      echo "[data-check] train-only SynID run must not use teacher_train_0 artifacts: ${data_dir}" >&2
      exit 2
    fi
  fi

  if [[ "${missing}" -ne 0 ]]; then
    echo "[data-check] expected generated train-only indexed data under: ${data_dir}" >&2
    exit 2
  fi
}

check_train_only_indexed_dataset "${DATA_DIR}"
...
RUN_TAG="generated-lora218-train-only-e${EPOCHS}-bs${BATCH_SIZE}-lr${LR}-G${GRAD_ACC}-N${GPUS_PER_NODE}-NN${NNODES}-kd${KD_RATIO}-${SYNID_KD_LOSS}-tau${SYNID_CONTRASTIVE_TAU}-a${SYNID_ALPHA}-b${SYNID_BETA}-${SYNID_LAYER_CONFIG}-${LAYER_TAG}-pool${SYNID_POOLING}-keywords-lambda${SYNID_SYNTAX_LAMBDA}-same-teacher-input-lora-${PEFT_LORA_R}-${PEFT_LORA_ALPHA}-${PEFT_LORA_DROPOUT}"
...
OPTS+=" --synid-use-privileged-teacher-input ${SYNID_USE_PRIVILEGED_TEACHER_INPUT}"
...
echo "  use privileged teacher input: ${SYNID_USE_PRIVILEGED_TEACHER_INPUT}"
echo "Length config:"
