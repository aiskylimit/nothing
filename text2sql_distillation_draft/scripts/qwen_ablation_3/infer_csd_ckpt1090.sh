...
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/qwen_ablation_3/csd/qwen_ablation_3}"
...
resolve_ckpt_path() {
  ...
  mapfile -t matches < <(
    find results/qwen3 \
      -path "*qwen_ablation_3_train_csd_generated_lora218*/${CKPT_STEP}" \
      -type d 2>/dev/null | sort
  )

  if [[ "${#matches[@]}" -eq 0 ]]; then
    echo "No qwen_ablation_3 csd checkpoint ${CKPT_STEP} found under results/qwen3." >&2
...
run_benchmark() {
  ...
  output_path="${output_dir}/qwen_ablation_3_train_csd_generated_lora218__ckpt${CKPT_STEP}__${INFER_SPLIT}__${db_name}_sql_result.json"
...
main() {
  ...
  echo "[infer] qwen_ablation_3 csd"
  echo "        ckpt=${ckpt_path}"
