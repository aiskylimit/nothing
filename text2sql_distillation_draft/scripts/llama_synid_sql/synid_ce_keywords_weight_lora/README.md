...
Llama SynID-SQL training on `synid_privileged_lora`.

Defaults:

- `DATA_DIR=processed_data/benchmarks/spider_data/synid_privileged_lora/llama`
...
```bash
RUN_GPUS=0 bash scripts/llama_synid_sql/synid_ce_keywords_weight_lora/train_g01.sh
RUN_GPUS=0 bash scripts/llama_synid_sql/synid_ce_keywords_weight_lora/train_g02.sh
```
