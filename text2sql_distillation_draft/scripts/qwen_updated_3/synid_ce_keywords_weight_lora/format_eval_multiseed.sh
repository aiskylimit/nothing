...
COLLECT_SCRIPT="${COLLECT_SCRIPT:-scripts/qwen_updated_3/synid_ce_keywords_weight_lora/collect_eval_results.py}"
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/synid_ce_keywords_weight_lora/qwen}"
EVAL_OUTPUT_ROOT="${EVAL_OUTPUT_ROOT:-results/eval/synid_ce_keywords_weight_lora/qwen}"
ETYPE="${ETYPE:-all}"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/qwen_updated_3/synid_ce_keywords_weight_lora/format_eval_multiseed.sh [benchmark ...]

Formats and evaluates multi-seed inference outputs under:
  results/infer/synid_ce_keywords_weight_lora/qwen/<benchmark>/seed*/

Benchmark args, if provided, must be one or more of:
  spider_data spider_syn spider_realistic spider_dk

Environment overrides:
  INFER_OUTPUT_ROOT     Default: results/infer/synid_ce_keywords_weight_lora/qwen
  EVAL_OUTPUT_ROOT      Default: results/eval/synid_ce_keywords_weight_lora/qwen
  ETYPE                 Default: all
