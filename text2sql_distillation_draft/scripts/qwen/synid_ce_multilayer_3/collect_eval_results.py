#!/usr/bin/env python3
"""Collect SynID multilayer grid eval logs into per-seed JSON summaries."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--infer-output-root",
        type=Path,
        default=Path("results/infer/synid_ce_multilayer_3/qwen"),
    )
    parser.add_argument(
        "--eval-output-root",
        type=Path,
        default=Path("results/eval/synid_ce_multilayer_3/qwen"),
    )
    parser.add_argument(
        "--required-benchmarks",
        nargs="+",
