...
COLLECT_SCRIPT="${COLLECT_SCRIPT:-scripts/qwen/synid_ce_multilayer_3/collect_eval_results.py}"
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/synid_ce_multilayer_3/qwen}"
EVAL_OUTPUT_ROOT="${EVAL_OUTPUT_ROOT:-results/eval/synid_ce_multilayer_3/qwen}"
ETYPE="${ETYPE:-all}"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/qwen/synid_ce_multilayer_3/format_eval_multiseed.sh [benchmark ...]

Formats and evaluates multi-seed inference outputs under:
  results/infer/synid_ce_multilayer_3/qwen/<benchmark>/seed*/

Benchmark args, if provided, must be one or more of:
  spider_data spider_syn spider_realistic spider_dk

Environment overrides:
  INFER_OUTPUT_ROOT     Default: results/infer/synid_ce_multilayer_3/qwen
  EVAL_OUTPUT_ROOT      Default: results/eval/synid_ce_multilayer_3/qwen
  ETYPE                 Default: all
