...
SynID-CE CSD sweep on Spider privileged-LoRA data with SQL keyword/schema token
...
```bash
bash scripts/qwen/synid_ce_keywords_weight_lora/run_full_pipeline.sh
```
...
```text
results/infer/synid_ce_keywords_weight_lora/qwen/<benchmark>/seed<seed>/<run>__ckpt<step>__test__full_sql_result.json
```

Evaluation summaries:

```text
results/eval/synid_ce_keywords_weight_lora/qwen/seed<seed>/eval_grid_results.json
results/eval/synid_ce_keywords_weight_lora/qwen/seed<seed>/best_grid_by_benchmark.json
results/eval/synid_ce_keywords_weight_lora/qwen/seed<seed>/best_grid_overall.json
```
