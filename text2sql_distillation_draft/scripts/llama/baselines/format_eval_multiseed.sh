...
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/llama_baselines/llama}"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/llama/baselines/format_eval_multiseed.sh [benchmark ...]

Benchmark args, if provided, must be one or more of:
  spider_data spider_syn spider_realistic spider_dk

Environment overrides:
  INFER_OUTPUT_ROOT     Default: results/infer/llama_baselines/llama
  EVAL_OUTPUT_ROOT      Default: results/eval/llama_baselines/llama
  ETYPE                 Default: all
