#!/usr/bin/env python3
"""Collect SynID no-keywords-weight LoRA-data grid eval logs into per-seed JSON summaries."""
...
DATA_TAG = "lora436"
DATA_DIR = "processed_data/benchmarks/spider_data/synid_privileged_lora_436/qwen"
...
def grid_config(grid_id: str) -> dict[str, Any]:
    ...
    index = grid_num - 1
    grid_total = len(LAYER_CONFIGS) * len(KD_RATIOS) * len(SYNID_CONTRASTIVE_TAUS) * len(SYNID_ALPHA_BETAS)
    if index < 0 or index >= grid_total:
        raise ValueError(f"grid_id out of range: {grid_id}")
    kd_index = index % len(KD_RATIOS)
    layer_index = (index // len(KD_RATIOS)) % len(LAYER_CONFIGS)
    tau_index = (index // (len(KD_RATIOS) * len(LAYER_CONFIGS))) % len(SYNID_CONTRASTIVE_TAUS)
    alpha_beta_index = index // (len(KD_RATIOS) * len(LAYER_CONFIGS) * len(SYNID_CONTRASTIVE_TAUS))
    layer_name, k, student_layers, teacher_layers = LAYER_CONFIGS[layer_index]
    alpha_beta = SYNID_ALPHA_BETAS[alpha_beta_index]
    return {
        "grid_id": grid_id,
        "data_tag": DATA_TAG,
        "data_dir": DATA_DIR,
        ...
        "beta": alpha_beta,
        "contrastive_tau": SYNID_CONTRASTIVE_TAUS[tau_index],
        "pooling": "sc",
        "pool_tau": 5.0,
...
def main() -> int:
    ...
    parser.add_argument(
        "--infer-output-root",
        type=Path,
        default=Path("results/infer/synid_ce_no_keywords_weight_lora_436/qwen"),
    )
    parser.add_argument(
        "--eval-output-root",
        type=Path,
        default=Path("results/eval/synid_ce_no_keywords_weight_lora_436/qwen"),
    )
    parser.add_argument(
        "--required-benchmarks",
        nargs="+",
