...
TARGET_KD_GRID_FILTERS=(
  scripts/qwen_updated/synid_ce_keywords_weight_lora_218/train_g01.sh
  scripts/qwen_updated/synid_ce_keywords_weight_lora_218/train_g02.sh
)
...
for grid_filter in "${TARGET_KD_GRID_FILTERS[@]}"; do
  bash running.sh \
    ...
    --filter "${grid_filter}" \
    --infer-after-train \
    --infer-script scripts/qwen_updated/synid_ce_keywords_weight_lora_218/infer_multiseed.py \
