...
check_conventional_indexed_dataset() {
  local data_dir="${1%/}"
  local missing=0
  ...
  if [[ "${missing}" -ne 0 ]]; then
    echo "[data-check] expected conventional indexed data under: ${data_dir}" >&2
    exit 2
  fi

  if [[ "${SYNID_USE_PRIVILEGED_TEACHER_INPUT}" =~ ^(1|true|yes|y)$ ]]; then
    if [[ ! -s "${data_dir}/teacher_train_0.bin" || ! -s "${data_dir}/teacher_train_0.idx" ]]; then
      echo "[data-check] privileged teacher input requested, but teacher_train_0 artifacts are missing in: ${data_dir}" >&2
      exit 2
    fi
  fi
}

check_conventional_indexed_dataset "${DATA_DIR}"
