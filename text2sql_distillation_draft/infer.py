...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Benchmark name under datasets/.",
    )
    parser.add_argument(
        "--split",
        default="dev",
...
def get_benchmark_paths(benchmark: str, split: str) -> Tuple[Path, Path]:
    if is_spider_style_benchmark(benchmark):
        ...
        if benchmark == "spider_data":
            ...
            benchmark_dir = Path("datasets/test") / benchmark
        else:
            benchmark_dir = Path("datasets/train") / benchmark
    ...
    if benchmark == "sparc":
        ...
        return Path("datasets") / benchmark / split_file, Path("datasets") / benchmark / "tables.json"
    ...
    if benchmark == "cosql_dataset":
        ...
        return (
            Path("datasets") / benchmark / "sql_state_tracking" / split_file,
            Path("datasets") / benchmark / "tables.json",
        )
