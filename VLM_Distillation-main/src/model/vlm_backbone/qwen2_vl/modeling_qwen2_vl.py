...
def _build_feature_masks(
    ...
    vision_feature_mask: torch.Tensor | None = None,
    attention_mask: torch.Tensor | None = None,
):
    if vision_feature_mask is None:
        vision_feature_mask = torch.zeros(inputs_embeds.shape[:2], dtype=torch.bool, device=inputs_embeds.device)
    else:
        vision_feature_mask = vision_feature_mask.to(device=inputs_embeds.device, dtype=torch.bool)

    if attention_mask is not None and attention_mask.dim() == 2:
        valid_token_mask = attention_mask.to(device=inputs_embeds.device, dtype=torch.bool)
        vision_feature_mask = vision_feature_mask & valid_token_mask
    else:
        valid_token_mask = torch.ones_like(vision_feature_mask, dtype=torch.bool, device=inputs_embeds.device)

    text_feature_mask = (~vision_feature_mask) & valid_token_mask
    has_vision = vision_feature_mask.any(dim=1, keepdim=True)
    last_vision_pos = vision_feature_mask.long().cumsum(dim=1).argmax(dim=1, keepdim=True)
    seq_positions = torch.arange(inputs_embeds.shape[1], device=inputs_embeds.device).unsqueeze(0)
    text_after_vision_mask = seq_positions > last_vision_pos
    text_feature_mask = torch.where(has_vision, text_feature_mask & text_after_vision_mask, text_feature_mask)

    return vision_feature_mask, text_feature_mask
...
class Qwen2VLModelOutputWithPast(ModelOutput):
    ...
    vision_feature_mask: torch.BoolTensor | None = None
    text_feature_mask: torch.BoolTensor | None = None
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            vision_feature_mask = image_mask[..., 0]
        ...
        if pixel_values_videos is not None:
            ...
            vision_feature_mask = video_positions if vision_feature_mask is None else vision_feature_mask | video_positions
            inputs_embeds = inputs_embeds.masked_scatter(video_mask, video_embeds)

        vision_feature_mask, text_feature_mask = _build_feature_masks(
            inputs_embeds, vision_feature_mask, attention_mask
        )
        ...
        output = Qwen2VLModelOutputWithPast(
            ...
            text_feature_mask=text_feature_mask,
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Qwen2VLCausalLMOutputWithPast(
            ...
            vision_feature_mask=outputs.vision_feature_mask,
            text_feature_mask=outputs.text_feature_mask,
        )
